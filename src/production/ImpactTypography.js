 ...
 */
export function pickPunchWord(sentenceText) {
    ...
    if (excl)
        return { word: excl.replace(/[.!]/g, ""), reason: "exclamation" };
    // 2) Numbers are punchy ("$1,000").
    const num = tokens.find(t => /^\$?\d[\d,.]*%?$/.test(t));
    if (num)
        return { word: num, reason: "number" };
    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
        ...
        if (POWER.includes(t))
            return { word: tokens[i].replace(/[.]$/, ""), reason: "power-word" };
    }
    // 4) Fallback: last meaningful word.
    for (let i = tokens.length - 1; i >= 0; i--) {
        if (isPunchWord(tokens[i]))
            return { word: tokens[i].replace(/[.,!?]$/, ""), reason: "last-punch" };
    }
    return null;
}
/**
 ...
 */
export function impactWordState(word, t, punchAt, energy = 0.5, beatIndex = 0) {
    if (!word)
        return null;
    const dt = t - punchAt;
    if (dt < -0.05)
...
function hash(s) { let h = 0; for (let i = 0; i < s.length; i++)
    h = (h * 31 + s.charCodeAt(i)) & 0xffff; return (h % 1000) / 1000; }
/** Render the impact word card as SVG (world-space, above the actors). */
export function renderImpactWord(state) {
    if (!state || state.opacity <= 0 || state.scale <= 0.01)
        return "";
