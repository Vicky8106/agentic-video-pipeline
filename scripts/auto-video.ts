...
import { renderAuthoredFrame, coverageGaps, type AuthoredFilm } from "../src/authored/AuthoredScene.js";
import { validateSheet, type SheetBeat } from "../src/director/DirectorSheet.js";
...
interface WorkerPlan {
  ...
  envelopeFile: string;
  fontFiles: string[];
  /** Forced-align sidecar for the shared word grid (same file, all workers). */
  wordSidecar?: string;
  /** Agent-authored director sheet (same file, all workers). */
  sheetFile?: string;
  /** Authored film module (film mode only; workers import it directly). */
  filmFile?: string;
}
 ...
 */
function loadAgentSheet(sheetArg: string, windowEnd: number): { beats: MovieBeat[]; director: string; warnings: string[] } {
  ...
  if (beats.length === 0) throw new Error(`Agent sheet ${p} has zero usable beats: ${warnings.join("; ")}`);
  return { beats, director: "agent-sheet", warnings };
}

async function resolveBeatSheet(
  windowEnd: number, sheetArg = "",
): Promise<{ beats: MovieBeat[]; director: string; warnings: string[] }> {
  if (!sheetArg) {
    throw new Error(
      "no direction: pass --sheet <beats.json> (agent-directed, check-sheet.ts green) " +
...
async function renderChunk(
  renderFrame: (t: number) => string, t0: number, t1: number, file: string, tag: string,
  width: number, height: number, fps: number, fontFiles: string[],
): Promise<void> {
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const baseFont = { defaultFontFamily: "Noto Sans", loadSystemFonts: false as const };
  ...
  for (let i = 0; i < frames; i++) {
    ...
    const pixels = new Resvg(svg, { fitTo: { mode: "width" as const, value: width }, font }).render().pixels;
...
function injectVoicePeaks(
  ...
  envelope: AudioEnvelope, finalDuration: number,
): number {
 ...
 */
function subsetFonts(scriptText: string, chunkDir: string): string[] {
  const dir = path.join(chunkDir, "fonts");
  try {
    const seen = new Set<string>();
    for (const ch of scriptText) {
      const cp = ch.codePointAt(0)!;
      if (cp >= 0x20 && cp <= 0x10ffff) seen.add(cp.toString(16).toUpperCase());
    }
    const ranges = [
    ...
    for (const f of FULL_FONTS) {
      ...
      if (r.status !== 0 || !fs.existsSync(target)) throw new Error(`pyftsubset failed for ${f}`);
      out.push(target);
    }
    ...
    console.warn(`[auto-video] font subsetting failed, using full fonts: ${(err as Error).message}`);
    return FULL_FONTS;
  }
}

function loadOrBuildEnvelope(audio: string, chunkDir: string, hopMs = 20): AudioEnvelope {
  ...
  try {
    for (const d of fs.readdirSync("/proc")) {
      ...
      try {
        ...
        if (m && Number(m[1]) === pid) kids.push(Number(d));
      } catch { /* raced exit */ }
    }
  } catch { /* no procfs: direct RSS only */ }
  for (const k of kids) sum += treeRss(k);
  return sum;
}

async function runPool(items: number[], jobs: number, spawnOne: (i: number) => ChildProcess): Promise<number> {
  const pending = [...items];
  ...
  const sample = () => {
    ...
    for (const c of running.values()) {
      if (c.pid) sum += treeRss(c.pid);
    }
    samples++;
    if (sum > peakMb) peakMb = sum;
  };
  await new Promise<void>((resolve) => {
    const pump = () => {
      sample();
      if (failed > 0) {
        for (const c of running.values()) try { c.kill(); } catch { /* already out */ }
      }
      if (pending.length === 0 && running.size === 0) return resolve();
      ...
      if (running.size > 0) setTimeout(pump, 2000);
      else resolve();
    };
    pump();
  });
...
async function workerMain(planFile: string, index: number): Promise<void> {
  const plan = JSON.parse(fs.readFileSync(planFile, "utf8")) as WorkerPlan;
  ...
  if (plan.filmFile) {
    const mod = await import(pathToFileURL(plan.filmFile).href) as { film: AuthoredFilm };
    const film = mod.film;
    await renderChunk((t) => renderAuthoredFrame(film, t),
      plan.bounds[index], plan.bounds[index + 1], plan.chunkFiles[index],
      tag, plan.width, height, plan.fps, plan.fontFiles);
    console.log(`[worker] CHUNK_COMPLETE ${plan.chunkFiles[index]}`);
    return;
  }
  const cues = loadScript(plan.script, plan.audioDur);
  const envelope = JSON.parse(fs.readFileSync(plan.envelopeFile, "utf8")) as AudioEnvelope;
  const sheet = plan.sheetFile
    ? validateSheet(JSON.parse(fs.readFileSync(plan.sheetFile, "utf8")), 0, plan.audioDur).beats as SheetBeat[]
    : [];
  ...
  }) as Parameters<typeof injectVoicePeaks>[0] & { transcript: { duration: number } };
  injectVoicePeaks(production, envelope, plan.finalDuration);
  await renderChunk(
    (t: number) => renderAutoSvgFrame({ production, timeSec: t, width: plan.width, height, impactWords: plan.impactWords }).svg,
    plan.bounds[index], plan.bounds[index + 1], plan.chunkFiles[index],
    tag, plan.width, height, plan.fps, plan.fontFiles);
  console.log(`[worker] CHUNK_COMPLETE ${plan.chunkFiles[index]}`);
}
 ...
 */
async function filmParentMain(): Promise<void> {
  ...
  const mod = await import(pathToFileURL(path.resolve(filmArg)).href) as {
    film: AuthoredFilm; WINDOW?: { start: number; end: number };
  };
  const film = mod.film;
  ...
  for (let i = 0; i < bounds.length - 1; i++) {
    ...
    if (ok) console.log(`[chunk ${i + 1}/${bounds.length - 1}] resume ${chunkFiles[i]}`);
    else todo.push(i);
  }
  if (todo.length > 0) {
    ...
    if (failed > 0) throw new Error(`${failed} chunk(s) failed`);
  }
  ...
  if (!keepChunks) {
    for (const f of [...chunkFiles, silent, listFile, planFile]) fs.rmSync(f, { force: true });
  }
...
async function parentMain(): Promise<void> {
  ...
  const sheetPre = sheetArg ? loadAgentSheet(sheetArg, audioDur).beats as SheetBeat[] : [];
  ...
  }) as Parameters<typeof injectVoicePeaks>[0] & { transcript: { duration: number }; plan: { resolved: unknown[] }; wordTiming: { source: string; count: number } };
  ...
  const comedyUnits = (production as unknown as { comedy: { units: Array<{ room: string | null; punchWord: string | null }> } }).comedy.units;
  ...
  for (const w of beatSheet.warnings.slice(0, 5)) console.warn(`[auto-video] director warning: ${w}`);
  ...
  const trimmed: AudioEnvelope = { ...envelope, values: envelope.values.slice(0, keep), duration: finalDuration };
  ...
  if (!keepChunks) {
    for (const f of [...chunkFiles, silent, listFile, planFile, envelopeFile]) fs.rmSync(f, { force: true });
  }
  const got = ffprobeDuration(outArg);
  let hwm = 0;
  try {
