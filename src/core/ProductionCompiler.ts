...
export function topicFor(text: string): VisualPlan["topic"] {
  const x = norm(text);
  if (/\b(\$|dollar|money|cost|price|million|billion|percent|%|salary|rent|wealth|crypto|bitcoin|budget|tax)\b|\d/.test(x)) return "money";
 ...
 */
export function compileProductionPlan(transcript: Transcript, direction: DirectionPlan, styleId: string): ProductionPlan {
