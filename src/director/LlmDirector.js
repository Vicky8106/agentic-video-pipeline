...
export function transcriptToInput(tr) {
...
export function buildDirectorPrompt(input) {
    ...
    const lines = input.sentences.map((s) => `[${s.start.toFixed(2)}-${s.end.toFixed(2)}] ${s.text}`);
    const user = [
...
export async function callDirectorLlm(prompt, opts) {
...
export function validateBeats(raw, windowStart, windowEnd) {
    const warnings = [];
    const beats = [];
    const list = [...raw].sort((a, b) => Number(a.startSec) - Number(b.startSec));
    list.forEach((r, i) => {
        const b = r;
        const actor = (b.actor ?? {});
        const cam = (b.camera ?? {});
        const prop = (b.activeProp ?? null);
        ...
        if (endSec <= startSec)
            return;
        const bgId = BG_IDS.includes(b.bgId) ? b.bgId : "BG-STUDIO";
        if (bgId !== b.bgId)
            warnings.push(`beat ${i + 1}: unknown bgId ${String(b.bgId)} -> BG-STUDIO`);
        const pose = POSES.includes(String(actor.pose)) ? String(actor.pose) : "neutral";
        if (pose !== actor.pose)
            warnings.push(`beat ${i + 1}: unknown pose ${String(actor.pose)} -> neutral`);
        const expression = EXPRESSIONS.includes(String(actor.expression)) ? String(actor.expression) : "deadpan_classic";
        if (expression !== actor.expression)
            warnings.push(`beat ${i + 1}: unknown expression -> deadpan_classic`);
...
function topicOf(text) {
    ...
    if (/\b(food|eat|dinner|lunch|breakfast|pizza|burger|bread|carb|coffee|diet|calorie)\b/.test(x))
        return "food";
    if (/\b(phone|text|social|internet|online|app|computer|screen|laptop|code|ai|camera)\b/.test(x))
        return "device";
    if (/\b(people|person|woman|man|celebrity|actor|actress|star|influencer|tiktok|instagram|boss|doctor|guy|girl)\b/.test(x))
        return "person";
    if (/\b(rule|law|contract|warning|sign|ban|policy|deadline)\b/.test(x))
        return "sign";
    if (/\b(before|after|versus|vs|than|more|less|higher|lower)\b/.test(x))
        return "compare";
    if (/\b(problem|wrong|fail|bad|danger|mistake|terrible|disaster)\b/.test(x))
        return "problem";
    return "explain";
}
/** Deterministic offline director: one beat per sentence group, no LLM. */
export function fallbackDirect(input) {
    const raw = [];
    let cur = null;
    const flush = () => { if (cur) {
        raw.push(cur);
    ...
    for (const s of input.sentences) {
        ...
        if (s.start - cur.start > 8 || /[!?]/.test(cur.text)) {
            flush();
            cur = { ...s };
            continue;
        }
