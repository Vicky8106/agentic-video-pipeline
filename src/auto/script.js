...
export function synthesizeCues(text, duration) {
    const sentences = splitSentences(text);
    if (sentences.length === 0)
        throw new Error("Script contains no sentences");
...
export function loadScript(scriptPath, audioDuration) {
    ...
    if (/\.(srt)$/i.test(scriptPath)) {
        ...
        if (cues.length === 0)
            throw new Error(`No cues parsed from ${scriptPath}`);
        return cues;
    }
    if (!(audioDuration > 0)) {
