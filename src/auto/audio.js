...
export function probeDuration(audioPath) {
    const out = execFileSync("ffprobe", ["-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", audioPath], { encoding: "utf8" }).trim();
    const d = Number(out);
    if (!Number.isFinite(d) || d <= 0)
        throw new Error(`Could not probe duration: ${audioPath}`);
    return d;
}
/** Decode to 16kHz mono PCM and take per-hop RMS, normalized to peak. */
export function buildEnvelope(audioPath, hopMs = 20) {
    ...
    try {
        ...
        if (dec.status !== 0)
            throw new Error(`ffmpeg decode failed for ${audioPath}`);
 ...
 */
export function findPeaks(env, threshold = 0.55, minGap = 2.0) {
    const peaks = [];
    let last = -Infinity;
