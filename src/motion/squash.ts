 ...
 */
export function squash(k: number): { scaleX: number; scaleY: number } {
  const sy = 1 + k;
  return { scaleX: 1 / Math.max(0.2, sy), scaleY: sy };
}

/** Impact squash from normalized impact speed v in [0,1]. */
export function impactSquash(v: number, amount = 0.3): { scaleX: number; scaleY: number } {
  const k = Math.min(1, Math.max(0, v));
  return squash(-amount * k);
}

/** Launch stretch from normalized charge c in [0,1]. */
export function launchStretch(c: number, amount = 0.25): { scaleX: number; scaleY: number } {
