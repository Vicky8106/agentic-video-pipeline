...
export type NumericPoseField = (typeof NUMERIC_POSE_FIELDS)[number];
 ...
 */
export function samplePose(keys: readonly PoseKey[], t: number): Pose {
...
export function withAnticipation(
  ...
  index: number,
  opts: { lead?: number; strength?: number } = {},
): PoseKey[] {
...
export function squashStretch(squash: number, stretch: number): { sx: number; sy: number } {
