 ...
 */
export function renderHandDrawnArrow(
  from: Point2D,
  to: Point2D,
  timeSec: number,
  label?: string
 ...
 */
export function renderHandDrawnCircle(
  center: Point2D,
  rx: number,
  ry: number,
  timeSec: number,
 ...
 */
export function renderActionLines(center: Point2D, radius: number, timeSec: number): string {
  const lines: string[] = [];
  const numLines = 14;
