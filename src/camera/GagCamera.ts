...
export function punchZoomBoost(t: number, punchAt: number | null, energy = 0.5): number {
  if (punchAt === null || !Number.isFinite(punchAt)) return 0;
  const dt = t - punchAt;
  if (dt < -PUNCH_ANTICIPATION_SEC || dt > PUNCH_DECAY_SEC) return 0;
  ...
  if (dt < 0) return peak * (1 + dt / 0.05) * 0.15;
 ...
 */
export function impactSquash(phase: number): { sx: number; sy: number } {
...
export function isReactionHoldWindow(
  t: number,
  beat: { start: number; end: number; role: string } | null | undefined,
  holdSec = 0.45,
): boolean {
  if (!beat) return false;
  if (beat.role !== "punchline" && beat.role !== "reaction") return false;
 ...
 */
export function freezeFactor(
  t: number,
  beat: { start: number; end: number; role: string } | null | undefined,
): number {
  if (!isReactionHoldWindow(t, beat)) return 0;
  if (!beat) return 0;
...
export function isHardCut(t: number, beatStart: number): boolean {
  return Math.abs(t - beatStart) < 1 / 24 + 1e-6;
}

/** Clamp a zoom into a grammar band (for audit, not for re-framing). */
export function inBand(zoom: number, band: keyof typeof GAG_ZOOM): boolean {
