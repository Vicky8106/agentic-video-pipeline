...
export function topicFor(text) {
    ...
    if (/\b(face|skin|body|weight|fat|thin|beauty|looks|appearance|jaw|cheek|wrinkle|aging|muscle|gym|workout|fitness)\b/.test(x))
        return "body";
    if (/\b(food|eat|dinner|lunch|breakfast|pizza|burger|bread|carb|coffee|beer|wine|diet|calorie|snack|restaurant)\b/.test(x))
        return "food";
    if (/\b(phone|text|message|social|internet|online|website|app|computer|screen|laptop|code|ai|software|camera|battery)\b/.test(x))
        return "device";
    if (/\b(car|tesla|vehicle|truck|bike|plane|airplane|flight|rocket|spaceship|train|drive|driving)\b/.test(x))
        return "vehicle";
    if (/\b(people|person|woman|man|celebrity|actor|actress|star|influencer|tiktok|instagram|boss|friend|doctor|cop|teacher|student|guy|girl)\b/.test(x))
        return "person";
    if (/\b(rule|law|contract|warning|sign|ban|policy|terms|agreement|deadline|license|notice)\b/.test(x))
        return "sign";
    if (/\b(book|clock|time|watch|timer|hammer|tool|trophy|medal|idea|lightbulb|gift|box)\b/.test(x))
        return "object";
    if (/\b(before|after|versus|vs|compared|than|more|less|increase|decrease|higher|lower)\b/.test(x))
        return "compare";
    if (/\b(first|second|third|step|process|because|therefore|so that|leads to|causes)\b/.test(x))
        return "process";
    if (/\b(problem|issue|wrong|fail|failed|bad|danger|risk|mistake|terrible|horrible|disaster)\b/.test(x))
        return "problem";
    return "explain";
}
function roleFor(s, index, count, globalIndex, totalCount) {
    const x = norm(s.text);
    const gi = globalIndex ?? index;
    const gc = totalCount ?? count;
    const isLast = gi === gc - 1;
    if (/[!?]/.test(s.text))
        return isLast || index === count - 1 ? "punchline" : "escalation";
    if (/\b(but|however|actually|suddenly|except|until|then|literally|somehow|apparently)\b/.test(x))
        return "escalation";
    if (/\b(because|therefore|means|causes|leads|so)\b/.test(x))
        return "explanation";
    // Short punchy fragments at a scene end land as punchlines.
    if (index === count - 1 && s.words.length <= 9)
        return "punchline";
    return index === 0 ? "setup" : "explanation";
}
function energy(text, role) {
    const x = norm(text);
    let e = 0.28;
    if (/[!?]/.test(text))
        e += 0.2;
    if (/\b(but|however|actually|suddenly|literally|obviously|apparently|meanwhile|except|until|then)\b/.test(x))
        e += 0.16;
    if (/\$|\d|%/.test(text))
        e += 0.12;
    if (/\b(terrible|insane|ridiculous|absurd|wild|crazy|horrible|genius|beautiful|ugly|massive|tiny)\b/.test(x))
        e += 0.18;
    if (role === "punchline" || role === "reaction")
        e += 0.15;
    return clamp(e);
}
function eventActions(events, start, end) {
    const out = [];
    for (const e of events) {
        ...
        if (e.kind === "expression")
            out.push(action("express", e.t, Math.min(end, e.t + (e.duration ?? 0.3)), { expression: e.payload?.expression ?? "deadpan_classic" }, actor));
        if (e.kind === "entrance")
            out.push(action("enter", e.t, Math.min(end, e.t + (e.duration ?? 0.28)), { side: e.payload?.side ?? "right" }, actor));
        if (e.kind === "morph")
            out.push(action("transform", e.t, Math.min(end, e.t + (e.duration ?? 0.6)), { to: e.payload?.to }, actor, e.target));
        if (e.kind === "prop")
            out.push(action("reveal", e.t, Math.min(end, e.t + (e.duration ?? 0.5)), { value: e.payload?.value, entrance: "pop" }, "host", e.target));
        if (e.kind === "shake")
            out.push(action("camera", e.t, Math.min(end, e.t + (e.duration ?? 0.18)), { move: "shake", strength: e.payload?.strength ?? 0.3 }));
        if (e.kind === "flash")
            out.push(action("camera", e.t, Math.min(end, e.t + (e.duration ?? 0.16)), { move: "impact", strength: e.payload?.strength ?? 0.3 }));
    }
    return out;
}
/**
