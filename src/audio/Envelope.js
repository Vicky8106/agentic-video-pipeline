...
export function amplitudeAt(env, t) {
    if (t < 0)
        return 0;
    const idx = t / env.hop;
    const i = Math.floor(idx);
    if (i >= env.values.length - 1)
        return env.values[env.values.length - 1] ?? 0;
    const f = idx - i;
 ...
 * between words.
 */
export function mouthOpenAt(env, t, opts = {}) {
...
export function isSpeakingAt(env, t, threshold = 0.06) {
...
export function buildBlinkPlan(seed, duration) {
...
export function blinkAmountAt(plan, t) {
    ...
    for (const s of plan.starts) {
        ...
        const local = t - s;
        if (local < plan.close)
            return local / plan.close;
        if (local < plan.close + plan.hold)
            return 1;
