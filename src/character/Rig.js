...
const num = (p, key, fallback) => {
    const v = p[key];
    return typeof v === "number" && Number.isFinite(v) ? v : fallback;
};
const bool = (p, key, fallback) => {
    const v = p[key];
    return typeof v === "boolean" ? v : fallback;
};
const str = (p, key, fallback) => {
    const v = p[key];
 ...
 */
export function ikTwoBone(origin, target, l1, l2, bendSign = 1) {
...
export function solveSkeleton(spec, body, pose) {
 ...
 */
export function renderRig(input) {
...
<g id="${id}" transform="translate(${x.toFixed(2)}, ${y.toFixed(2)}) rotate(${rotation.toFixed(2)}) scale(${(scale * sx).toFixed(4)}, ${(scale * sy).toFixed(4)})" opacity="${alpha}">
  ...
  <g stroke-linecap="round" stroke-linejoin="round" fill="none">
    ...
    function hipL() { return { x: sk.hip.x - body.hipWidth * 0.22, y: sk.hip.y }; }
    function hipR() { return { x: sk.hip.x + body.hipWidth * 0.22, y: sk.hip.y }; }
}
