 ...
 */
export function castForBeat(beatIndex, text = "") {
 ...
 */
export function beatHasCoStar(role, beatIndex) {
    if (role === "punchline" || role === "reaction")
        return true;
    if (role === "setup")
        return beatIndex % 2 === 0;
    if (role === "escalation" || role === "reveal")
        return beatIndex % 3 !== 2;
...
export function computeCoStarPresence(beats) {
    ...
    for (const b of beats) {
        ...
        if (!has && (gapBeats >= MAX_GAP_BEATS - 1 || gapSec + (b.end - b.start) >= MAX_GAP_SEC))
            has = true;
        if (has) {
            present.add(b.id);
            gapBeats = 0;
...
function timed05(t) { return Math.sin(t * 8.5); }
function smoothstep(x) { const c = clamp(x); return c * c * (3 - 2 * c); }
