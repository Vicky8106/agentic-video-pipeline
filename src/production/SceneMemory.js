...
export function buildStageObjects(beats, opts = {}) {
    ...
    for (const b of beats) {
        ...
        for (const concept of concepts) {
            ...
            const FLANKS = [PAIR_POS[1], PAIR_POS[0], CENTER_POS];
            const visible = () => objects.filter((o) => o.retireAt > b.start + 1.0);
            const free = (px) => !visible().some((o) => Math.abs(o.x - px) < 150);
            let guard = 0;
            while (!FLANKS.some(([fx]) => free(fx)) && guard++ < 4) {
                let earliest = null;
                for (const o of visible()) {
                    if (!earliest || o.retireAt < earliest.retireAt)
                        earliest = o;
                }
                if (!earliest)
                    break;
            ...
            if (kind === "vehicle" && LIFT_CONTEXT.test(b.semantic))
                continue;
