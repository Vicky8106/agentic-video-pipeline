...
import { Sentence } from "../subtitles/Transcript";
...
const SCENE_FOCAL_BEATS: Record<string, FocalSegment[]> = {
  scene_01_ecosystem: [
...
export class CameraAgent {
  ...
  public updateFrame(
    ...
    slamPunchAt: number | null,
    outputCameraTarget?: { x: number; y: number; zoom: number; isCut?: boolean }
  ) {
  ...
  private applySmoothDynamics(t: number, activeSentence: Sentence | null, slamPunchAt: number | null) {
  ...
  private resolveBlueprintTarget(sceneId: string, sceneTime: number): FocalSegment {
