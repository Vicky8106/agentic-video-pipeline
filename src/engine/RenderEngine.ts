...
import { DirectionPlan } from "../director/Director";
import { Performance, secondaryMotion } from "../director/Performance";
import { cameraAt, viewBoxFor, pxPerUnit, traumaAt, shakeAt, flashAt, ImpactEvent } from "../camera/CameraTrack";
import { renderRig } from "../character/Rig";
import { CharacterSpec } from "../character/CharacterSpec";
import { Envelope, mouthOpenAt, isSpeakingAt, BlinkPlan, blinkAmountAt } from "../audio/Envelope";
import { Transcript, wordAt } from "../subtitles/Transcript";
...
export interface EngineInput {
  ...
  specs: Record<string, CharacterSpec>;
  blinkPlans: Record<string, BlinkPlan>;
  hostId?: string;
  width?: number;
  height?: number;
  fps?: number;
...
export interface Frame {
  ...
  sceneId: string;
  subtitle: string;
  /** Impact intensity, for diagnostics. */
  trauma?: number;
}
...
const phaseOf = (id: string, seed: number): number => seed * Math.PI * 2 + id.length * 0.7;

function backgroundFor(t: number, sceneLabel: string, w = 1920, h = 1080): string {
...
function subtitleLayer(t: number, transcript: Transcript, w = 1920, h = 1080): string {
...
let impactCache: { key: object; impacts: ImpactEvent[] } | null = null;

function impactsFor(plan: DirectionPlan): ImpactEvent[] {
...
export function renderFrame(engine: EngineInput, t: number): Frame {
  ...
  const ids = new Set<string>([hostId, ...perf.ids]);
  ...
  for (const id of ids) {
    ...
    if (
      home[0] < vbNums[0] - 320 || home[0] > vbNums[0] + vbNums[2] + 320 ||
    ...
    figures.push(
      renderRig({
        ...
        handLag: sec.handLag,
        hairLag: sec.hairLag,
        mouthDrive: life.mouthOpen as number,
        blinkAmount: blink,
      }),
    );
  }
