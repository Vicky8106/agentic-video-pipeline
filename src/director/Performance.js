 ...
 */
import { samplePose, withAnticipation } from "../anim/Pose";
import { ramp, springFrom, SPRING_DEFAULTS } from "../anim/Track";
import { BODY_PRESETS, sampleBody } from "../character/CharacterSpec";
...
const standingHipY = (body) => FLOOR_Y - (body.thighLen + body.shinLen) * 0.97;
/**
 * Build the performance from a direction plan.
 */
export function buildPerformance(plan, opts = {}) {
    ...
    const framedUntil = new Map();
    for (const shot of plan.shots) {
        if (!shot.tag || shot.tag.startsWith("morph:") || shot.tag.startsWith("readout:"))
            continue;
    ...
    for (const ev of plan.events) {
        ...
        if (ev.kind === "expression") {
            ...
            const until = Math.min(plan.duration, Math.max(framedUntil.get(target) ?? ev.t + (ev.duration ?? 1.5), ev.t + (ev.duration ?? 1.5)) + 0.25);
            ...
            const preset = BODY_PRESETS[to] ?? (["leaner", "thinner", "skinnier", "snatched"].includes(to) ? BODY_PRESETS.lean
    ...
    for (const a of actors.values()) {
        ...
        for (const k of a.poseKeys) {
            if (typeof k.pose.x !== "number")
                k.pose.x = a.home[0];
            if (typeof k.pose.y !== "number")
                k.pose.y = a.home[1];
        }
        a.poseKeys.sort((x, y) => x.t - y.t);
        a.bodyKeys.sort((x, y) => x.t - y.t);
        if (a.bodyKeys.length === 1)
            a.bodyKeys.push({ t: plan.duration, body: a.bodyKeys[0].body });
    }
 ...
 */
export function secondaryMotion(poseAt, t, dt) {
    const now = poseAt(t);
...
export function landingSquash(t, landAt) {
    if (t < landAt)
