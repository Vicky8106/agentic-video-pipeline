/**
 * Crash hunter: render 43.7-60s exactly as render-scenes.ts does, writing
 * each frame's SVG to /tmp/crash_probe.svg BEFORE rasterizing. If resvg
 * aborts, the last file on disk is the culprit frame.
 */
...
const root = "/root/casually_engine_v7";
const camera = new Camera(1280, 720);
const srtContent = fs.readFileSync(root + "/public/0-chapter-1.srt", "utf8");
...
const START = parseFloat(process.argv[2] ?? "43.7");
const END = parseFloat(process.argv[3] ?? "60");
const prevTargetRef = { key: "", lastChange: START };
const fontFiles = ["/usr/share/fonts/truetype/noto/NotoSans-Bold.ttf"];

for (let i = 0; i < (END - START) * 24; i++) {
  const t = START + i / 24;
  ...
  if (hold > 1.2) camera.current.zoom *= 1 + 0.008 * Math.min(1, (hold - 1.2) / 2) * Math.sin(hold * 2 * Math.PI / 11);
  const sent = transcript.sentences.find(s => t >= s.start && t <= s.end) ?? transcript.sentences.slice().reverse().find(s => s.start <= t);
  let push = 1;
  if (sent) {
    const bp = Math.min(1, Math.max(0, (t - sent.start) / Math.max(0.4, sent.end - sent.start)));
    push = 1 + 0.06 * (1 - (1 - bp) * (1 - bp));
  }
  ...
  <defs><filter id="cardShadow" x="-10%" y="-10%" width="120%" height="120%"><feDropShadow dx="0" dy="8" stdDeviation="12" flood-color="#000000" flood-opacity="0.18"/></filter>${IMPACT_DEFS}</defs>
  ...
  fs.writeFileSync("/tmp/crash_probe.svg", `<!-- t=${t.toFixed(4)} scene=${scene.id} viewBox=${viewBox} -->\n` + svg);
  try {
    new Resvg(svg, { fitTo: { mode: "width", value: 1280 }, font: { fontFiles, defaultFontFamily: "Noto Sans", loadSystemFonts: false } }).render();
  } catch (e) {
    console.log(`JS-CAUGHT at t=${t.toFixed(3)}: ${String(e).slice(0, 100)}`);
    process.exit(2);
  }
  if (i % 48 === 0) console.log(`ok t=${t.toFixed(1)} vb=${viewBox.slice(0, 40)}`);
}
console.log(`range ${START}-${END} clean`);
