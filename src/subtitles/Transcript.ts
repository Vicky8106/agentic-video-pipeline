...
import { SubtitleItem } from "./SrtParser";
...
export function splitGluedWords(input: string): string {
  return input
...
export function splitLeadingFunctionWords(input: string, vocabulary?: ReadonlySet<string>): string {
  return input
    .split(" ")
    .map((word) => {
...
export function normalizeCueText(raw: string, vocabulary?: ReadonlySet<string>): string {
...
export interface Word {
  text: string;
  /** Lowercased alphanumeric core, for matching. */
  token: string;
  start: number;
  end: number;
  cueIndex: number;
...
function wordWeight(surface: string): number {
 ...
 */
export function buildWordTimings(cues: readonly SubtitleItem[]): Word[] {
  const words: Word[] = [];
  let ordinal = 0;
  let sentenceIndex = 0;
...
export function buildSentences(words: readonly Word[]): Sentence[] {
  const out: Sentence[] = [];
  let bucket: Word[] = [];
  const flush = () => {
...
export function wordAt(words: readonly Word[], t: number): Word | null {
...
export function buildTranscript(cues: readonly SubtitleItem[]): Transcript {
