 ...
 */
export function auditDirectionPlan(plan, transcript) {
    const sceneAudits = [];
    let totalScore = 100;
    let macroCount = 0;
    for (const scene of plan.scenes) {
        const sceneShots = plan.resolved.filter((s) => s.start >= scene.start - 0.05 && s.end <= scene.end + 0.05);
        const sceneEvents = plan.events.filter((e) => e.t >= scene.start - 0.05 && e.t <= scene.end + 0.05);
        ...
        const notes = [];
        if (hasMacro)
            notes.push(`Macro punchline present (zoom: ${maxZoom.toFixed(2)}x)`);
        if (sceneShots.some((s) => s.move === "whip"))
            notes.push("Whip-pan kinetic transition");
        if (sceneShots.some((s) => s.kind === "reaction"))
            notes.push("Host reaction cut included");
        if (antiSlideshowPassed)
            notes.push("Anti-slideshow motion passed (< 3.2s static limit)");
        sceneAudits.push({
    ...
    const avgDuration = plan.resolved.reduce((acc, s) => acc + (s.end - s.start), 0) /
