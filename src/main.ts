import "./style.css";
import { parseSrt, getActiveSubtitle, SubtitleItem } from "./subtitles/SrtParser";
...
// Element hooks
const svg = document.querySelector<SVGSVGElement>("#animation-svg")!;
const sceneBgLayer = document.querySelector<SVGGElement>("#scene-background")!;
const sceneCharLayer = document.querySelector<SVGGElement>("#scene-character")!;
const sceneFgLayer = document.querySelector<SVGGElement>("#scene-foreground")!;
const subtitleBox = document.querySelector<HTMLDivElement>("#subtitle-box")!;
const subtitleText = document.querySelector<HTMLDivElement>("#subtitle-text")!;
...
const timeReadout = document.querySelector<HTMLSpanElement>("#time-readout")!;
const statsReadout = document.querySelector<HTMLDivElement>("#stats-readout")!;
...
const btnBack5 = document.querySelector<HTMLButtonElement>("#btn-back-5s")!;
const btnFwd5 = document.querySelector<HTMLButtonElement>("#btn-fwd-5s")!;
...
function tick(timestamp: number) {
  ...
  if (activeSub) {
    ...
    const highlighted = activeSub.cleanText.replace(
      /(Thicc|stick|Jenna Ortega|Emma Stone|Ariana Grande|unsubscribe|lunch|Tim Burton|PS1 graphics|Heroin Chick|Kate Moss|Diet Coke|apathy|Victorian|BBL|Pixar Mom|squats|hourglass|body positivity|fidget spinners|podcast|pharmaceutical|boss|Y2K|low-rise|Miu Miu|two thousand dollars|digestive tract)/gi,
...
(window as any).__seekAnimation = (t: number) => {
  virtualTime = t;
