...
import { CharacterSpec, BodyShape, BASE_BODY, mixBody } from "./CharacterSpec";
import { Vec2, solveSkeleton, Skeleton, ikTwoBone } from "./Rig";
...
export interface ProgrammableFaceState {
  // Eyebrows (Independent Asymmetric Control)
  eyebrowLeftHeight?: number;   // -15px to +25px
  eyebrowRightHeight?: number;  // -15px to +25px
  eyebrowLeftTilt?: number;     // -20 deg (furrowed/angry) to +20 deg (skeptical/worried)
  eyebrowRightTilt?: number;    // -20 deg to +20 deg
  eyebrowArch?: number;         // 0.0 (flat straight) to 1.0 (dramatic arch)
  
  // Eyes & Eyelids
  gazeX?: number;               // -1.0 (full left) to +1.0 (full right)
  gazeY?: number;               // -1.0 (looking up) to +1.0 (looking down)
  eyelidLeftOpen?: number;      // 0.0 (closed/blink) to 1.0 (normal) to 1.8 (eye-pop)
  eyelidRightOpen?: number;     // 0.0 to 1.8
  pupilDilation?: number;       // 0.4 (pinprick shock) to 1.5 (cute anime)
  eyeVariant?: EyeVariant;
  
  // Mouth Engine
  mouthSmile?: number;          // -1.0 (sad frown) to 0.0 (flat deadpan) to +1.0 (wide grin)
  mouthOpen?: number;           // 0.0 (closed) to 1.0 (speaking/shouting) to 1.5 (jaw drop)
  mouthWobble?: number;         // 0.0 (clean line) to 1.0 (nervous/cringe squiggly tremor)
  showTeeth?: boolean;          // Display signature Casually Explained rectangular teeth
  
  // Head Orientation
  headTilt?: number;            // -30 deg to +30 deg
  headYaw?: number;             // -1.0 (facing left) to +1.0 (facing right)
}

export interface ProgrammablePostureState {
  // Spine & Breathing
  spineLean?: number;           // -25 deg (slouch/defeat) to +20 deg (confident lean)
  chestRise?: number;           // Continuous breath oscillation (0 to 6 px)
  bodySway?: number;            // Weight transfer horizontal offset (-10 to +10 px)
  hipShift?: number;            // Pelvis lateral slide
  
  // Arms Forward Kinematics (degrees)
  leftArmAngle1?: number;       // Shoulder angle
  leftArmAngle2?: number;       // Elbow bend
  rightArmAngle1?: number;
  rightArmAngle2?: number;
  
  // Inverse Kinematics Targets (Local or World Space)
  leftHandTarget?: Vec2;
  rightHandTarget?: Vec2;
  
  // Legs Forward Kinematics (degrees)
  ...
  // Squash & Stretch
  squash?: number;              // 0.7 (compressed) to 1.3 (stretched)
  stretch?: number;
}

export interface ProgrammablePuppetState extends ProgrammableFaceState, ProgrammablePostureState {
  id?: string;
  x?: number;
  y?: number;
  scale?: number;
  ...
  // Dynamic Body Customization
  customBody?: Partial<BodyShape>;
...
export interface PuppetSockets {
  headCenter: Vec2;
  eyeCenter: Vec2;
  leftHand: Vec2;
  rightHand: Vec2;
...
export interface RenderPuppetResult {
  svg: string;
  sockets: PuppetSockets;
}
...
export function lookAt(
  puppetWorldPos: Vec2,
  targetWorldPos: Vec2,
  headWorldYOffset = -180,
): { gazeX: number; gazeY: number; headYaw: number; headTilt: number } {
...
export function renderProgrammablePuppet(
  spec: CharacterSpec,
  state: ProgrammablePuppetState,
  options: {
    pxPerUnit?: number;
    strokePx?: number;
    timeSec?: number;
  ...
  const body: BodyShape = state.customBody ? { ...spec.body, ...state.customBody } : spec.body;
  
  // Solve full articulated skeleton
  const poseRecord: Record<string, unknown> = { ...state };
  const sk: Skeleton = solveSkeleton(spec, body, poseRecord);
  ...
  const renderEye = (cx: number, cy: number, open: number, side: "left" | "right"): string => {
    if (open <= 0.1) {
      // Blink / Closed eye
