...
export interface AuthoredScene {
  id: string;
  /** Absolute film times, seconds. Adjacent scenes must tile, not overlap. */
  start: number;
  end: number;
  /** Director's motive: which joke/beat this serves, why this staging. */
  intent: string;
  renderWorld: (t: number) => string;
  camera: (t: number) => AuthoredCamera;
}
...
export const span = (t: number, a: number, b: number): number =>
  b <= a ? (t >= b ? 1 : 0) : clamp((t - a) / (b - a), 0, 1);

// ---------------------------------------------------------------------------
// Camera-move library. Parametric primitives the directing agent composes
// per scene — the library grows by adding primitives here, never by picking
// moves at render time. Every composed camera still carries the scene's
// `intent`, which is what keeps a large vocabulary from reading as random.
// ---------------------------------------------------------------------------
...
export function keyframedCamera(frames: CamKeyframe[]): (t: number) => AuthoredCamera {
  const sorted = [...frames].sort((a, b) => a.at - b.at);
  return (t: number): AuthoredCamera => {
    if (t <= sorted[0].at) return { x: sorted[0].x, y: sorted[0].y, zoom: sorted[0].zoom };
...
export function withPunch(
  base: (t: number) => AuthoredCamera,
  punchAt: number,
  peak = 0.38,
): (t: number) => AuthoredCamera {
  return (t: number): AuthoredCamera => {
    const c = base(t);
    const dt = t - punchAt;
    let bump = 0;
    if (dt >= -0.25 && dt < -0.05) bump = -0.04 * Math.sin(Math.PI * (dt + 0.25) / 0.2);
    else if (dt >= -0.05 && dt < 0) bump = peak * 0.15 * (1 + dt / 0.05);
    else if (dt >= 0 && dt < 1.0) {
...
export function coverageGaps(film: AuthoredFilm, start: number, end: number): Array<{ start: number; end: number }> {
  ...
  if (cursor < end - 1e-6) gaps.push({ start: cursor, end });
  return gaps;
}

export function sceneAt(film: AuthoredFilm, t: number): AuthoredScene | null {
...
export function renderAuthoredFrame(film: AuthoredFilm, t: number): string {
  const scene = sceneAt(film, t);
  if (!scene) throw new Error(`no authored scene covers t=${t.toFixed(3)}s`);
