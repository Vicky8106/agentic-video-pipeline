/**
 * ComedyStructure: the drop-in brain. Timed transcript in, joke structure out.
 *
 * No API key, no network, no per-script hours. It recovers what a director
 * reads from a script: where each joke unit starts and ends (~10-15s
 * setup -> punchline cycles), which sentence is the punchline, the exact
 * punch word inside it, who is in the scene, and where the scene lives.
 *
 * Signals (all deterministic, all inspectable):
 * - Rhythm: a short sentence after a long windup is the oldest punch shape
 *   in comedy. Length contrast is the strongest single signal.
 * - Discourse pivots: but/however/actually/suddenly/until/then/literally.
 * - Terminal punctuation: ! and ? close thoughts; ? + answer = call/response.
 * - Sentiment flip: the punch turns the setup's valence (lexicon below).
 * - Callback: a content phrase repeated from an earlier unit.
 * - Rule of three: third item in a list run terminates the bit.
 * - Entities: capitalized non-initials + repeated content nouns; pronouns
 *   attach to the running entity (no full coreference, disclosed).
 * - Rooms: location nouns per unit, sticky across units unless moved.
 *
 * Output shapes feed the existing machinery (roles, punch words, roster,
 * stable rooms). Where evidence is thin the analyzer abstains (null) and
 * the legacy heuristics apply — never a confident wrong answer.
 */
import type { Transcript, Sentence } from "./Transcript.js";
...
export interface SentenceNote {
  ...
  role: "setup" | "escalation" | "punchline";
  /** Exact word the punch lands on (surface form), or null. */
...
export interface JokeUnit {
  id: number;
  start: number;
  end: number;
  sentences: number[];
  /** End time of the setup span (punchline starts after this). */
  setupEnd: number;
  punchSentence: number;
  punchWord: string | null;
  entities: string[];
  room: string | null;
}

export interface ComedyAnalysis {
  units: JokeUnit[];
  /** Per-sentence notes by sentence index. */
  notes: Map<number, SentenceNote>;
}

const norm = (s: string): string => s.toLowerCase();
const wordCount = (s: Sentence): number => s.words.length;
...
export function entitiesIn(text: string, sentenceStart: boolean): string[] {
  const out: string[] = [];
  ...
  for (const raw of text.split(/[^A-Za-z0-9'’-]+/)) {
    ...
    if (STOP_ENT.has(lower) || seen.has(lower)) continue;
    // Long -ly adverbs are never entities ("affectionately").
    if (/[a-z]{3,}ly$/.test(lower) && lower.length > 8) continue;
    // Reflexives are never entities ("themselves").
    if (/^(myself|yourself|himself|herself|itself|ourselves|themselves)$/.test(lower)) continue;
    // Fused transcript garbage ("likethey're", "yourfriend's").
    if (/^[a-z]+'[a-z]+$/.test(lower) && lower.length > 8) continue;
...
function roomFor(text: string): string | null {
  for (const [re, bg] of ROOMS) if (re.test(text)) return bg;
  return null;
}

function zeroEvidence(): PunchEvidence {
...
function scorePunch(
  s: Sentence, si: number, unit: Sentence[], seenPhrases: Set<string>,
  isScriptCloser = false,
): PunchEvidence {
  ...
  if (prev >= 12 && n <= 8) e.rhythm = 2;
  else if (prev >= 10 && n <= 6) e.rhythm = 2;
  else if (prev >= 8 && n <= Math.max(4, Math.floor(prev * 0.6))) e.rhythm = 1;
  else if (n <= 5 && si === unit.length - 1 && unit.length > 1) e.rhythm = 1;
  if (PIVOTS.test(s.text)) e.pivot = 2;
  if (EXCLAIM.test(s.text)) e.terminal = 2;
  else if (QUESTION.test(s.text)) e.terminal = 1;
  // Closers land: the final button of the script punches by position.
  if (isScriptCloser && wordCount(s) <= 10) e.terminal += 2;
  // Call/response: question just before + this answers it.
  if (si > 0 && QUESTION.test(unit[si - 1].text) && !QUESTION.test(s.text)) e.terminal += 1;
  ...
  if (si > 0) {
    ...
    if (before !== 0 && now !== 0 && Math.sign(before) !== Math.sign(now)) e.flip = 2;
    else if (before === 0 && Math.abs(now) >= 2) e.flip = 1;
  }
  ...
  for (let k = 0; k < toks.length - 1; k++) {
    if (seenPhrases.has(`${toks[k]} ${toks[k + 1]}`)) { e.callback = 2; break; }
  }
  // Rule of three: third comma-item sentence terminates.
  if (si === 2 && unit.length === 3) e.ruleOfThree = 1;
...
function pickPunchWord(s: Sentence): string | null {
  const words = s.words;
  ...
  for (let i = words.length - 1; i >= 0; i--) {
    ...
    if (core.length >= 4 && !STOP_ENT.has(core)) return words[i].text;
  }
  void piv;
 ...
 */
export function analyzeComedy(tr: Transcript): ComedyAnalysis {
  const notes = new Map<number, SentenceNote>();
  const units: JokeUnit[] = [];
  const seenPhrases = new Set<string>();
  const freq = new Map<string, number>();
  for (const s of tr.sentences) {
    for (const e of entitiesIn(s.text, true)) freq.set(e, (freq.get(e) ?? 0) + 1);
  }
  ...
  const flush = () => {
    ...
    for (const s of cur) {
      for (const e of entitiesIn(s.text, true)) ents.set(e, (ents.get(e) ?? 0) + 1);
    }
    ...
    for (const s of cur) {
      const toks = norm(s.text).replace(/[^a-z ]/g, " ").split(/\s+/).filter((t) => t.length > 4);
      for (let k = 0; k < toks.length - 1; k++) seenPhrases.add(`${toks[k]} ${toks[k + 1]}`);
    }
  ...
  for (const s of tr.sentences) {
    ...
    if (prev && (gap > 1.2 || s.start - cur[0].start > 12)) flush();
    cur.push(s);
    // Hard punch signals close the unit immediately.
    if (EXCLAIM.test(s.text) || (PIVOTS.test(s.text) && wordCount(s) <= 10)) flush();
  }
  flush();
  return { units, notes };
}
