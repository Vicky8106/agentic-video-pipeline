 ...
 */
export function popInSquash(t, duration = 0.25, basePos = { x: 0, y: 0 }) {
    if (t <= 0) {
 ...
 */
export function slamDrop(t, dropDuration = 0.16, basePos = { x: 0, y: 0 }, startYOffset = 400) {
    if (t <= 0) {
        return {
            x: basePos.x,
 ...
 */
export function floatingHover(timeSec, freq = 2.0, amplitude = 6.0) {
 ...
 */
export function harmonicSwing(timeSec, freq = 1.4, maxAngle = 35) {
 ...
 */
export function vortexSwirl(t, duration = 0.8, startPos = { x: 0, y: 0 }, targetPos = { x: 0, y: 0 }) {
    if (t <= 0) {
 ...
 */
export function particleBurst(t, count = 8, duration = 0.5, maxRadius = 160) {
    if (t <= 0 || t >= duration)
        return [];
    const p = t / duration;
    const out = [];
