...
async function renderChunk(production, t0, t1, file, tag, width, height, fps, fontFiles, impactWords) {
...
async function workerMain(planFile, index) {
    ...
    const production = createAutoProduction(toSrt(cues), plan.style, { envelope });
    injectVoicePeaks(production, envelope, plan.finalDuration);
    const height = Math.round((plan.width * 9) / 16);
    await renderChunk(production, plan.bounds[index], plan.bounds[index + 1], plan.chunkFiles[index], `chunk ${index + 1}/${plan.bounds.length - 1}`, plan.width, height, plan.fps, plan.fontFiles, plan.impactWords);
...
async function parentMain() {
    ...
    const production = createAutoProduction(srtText, style, { envelope });
